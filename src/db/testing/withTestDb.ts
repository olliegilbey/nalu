 ...
 */
// Keep this list in sync with schema/index.ts: adding a table without updating
// it silently leaves stale rows between tests. Consider deriving from schema
// keys (e.g. via Object.values(schema).map(t => getTableName(t))) if drift
// becomes a problem; explicit list is fine while the schema is small.
