 ...
 *
 * Output contract branches on `inputs.outputContract` (default `"json"`):
 * - `"json"` (blocking tRPC rollback path): single JSON per
 *   `waveMidTurnSchema` / `makeWaveCloseSchema` every turn.
 * - `"tools"` (streaming transport): mid-turn prose is plain text, structured
 *   actions are tool calls (`recordComprehensionSignals` /
 *   `presentQuestionnaire`); the close turn stays single-JSON. One prompt
 *   covers the whole Wave so the cache prefix survives the mid→close switch.
 */
export function renderTeachingSystem(inputs: WaveSeedInputs): string {
  const contract: WaveOutputContract = inputs.outputContract ?? "json";
 ...
 * The only contract-dependent copy is the questionnaire channel's name: the
 * mega-schema's `questionnaire` field (json) vs the `presentQuestionnaire`
 * tool (tools). Everything else is byte-identical across contracts.
 */
function renderRoleBlock(contract: WaveOutputContract): string {
  const channel =
    contract === "tools" ? "the presentQuestionnaire tool" : "the questionnaire field";
  return `You are Nalu, an expert teacher and tutor. You teach in short bite-sized lessons — each lesson is about ten turns of dialogue, roughly five minutes for the learner. Keep the energy warm and the learner always with you.
...
/** Sentence-lead helper for the channel noun ("the …" → "The …"). */
function capitalise(s: string): string {
  return s.charAt(0).toUpperCase() + s.slice(1);
}

/**
 * Output-format block per contract. `json` is the pre-tool-calling text,
 * byte-for-byte. `tools` describes the mid-turn tool channel AND pins the
 * close turn back to single-JSON — the harness provides no tools on that
 * turn (`executeWaveClose` runs the blocking `executeTurn` with
 * `response_format`), so the prompt must cover both turn shapes.
 */
const OUTPUT_FORMAT_BLOCKS: Record<WaveOutputContract, string> = {
  json: `You respond with a single JSON object validated against the schema provided each turn. Do not emit XML tags or other framing. The schema describes every field, when each is required, and what each must contain.`,
